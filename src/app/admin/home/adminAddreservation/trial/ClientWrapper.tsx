...
import styles from './page.module.css'; // 相対パスに注意
...
export default function ClientWrapper() {
  ...
  useEffect(() => {
    if (!teacherId) {
      alert('講師IDが見つかりません。管理画面から正しく遷移してください。');
  ...
  const handleAddSchedule = async () => {
    ...
    alert(`${successCount}件の体験クラス予約枠を登録しました`);
    setSelectedDates([]);
    setTime('');
    setLessonType('');
    setMemo('');
  ...
  return (
    <main className={styles.container}>
      <h1 className={styles.heading}>体験スクールの追加</h1>
